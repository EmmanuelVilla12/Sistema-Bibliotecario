...
const FILTROS_CONFIG = {
  libros: [
    ...
    { key: "fecha_publicacion", label: "Año publicación",  placeholder: "ej. 1967" },
  ],
  autores: [
    { key: "nombre",       label: "Nombre",        placeholder: "ej. García Márquez" },
    { key: "nacionalidad", label: "Nacionalidad",   placeholder: "ej. Colombiana" },
  ],
  usuarios: [
    { key: "nombre",       label: "Nombre",         placeholder: "ej. Juan Pérez" },
    { key: "tipo_usuario", label: "Tipo de usuario", placeholder: "ej. Estudiante" },
  ],
  prestamos: [
...
function App() {
  ...
  const formulario = () => {
    if (tabla === "libros") return (
      <form onSubmit={(e) => {
        e.preventDefault();
        crear({ nombre: e.target.nombre.value, id_autor: Number(e.target.autor.value), fecha_publicacion: Number(e.target.fecha.value), stock: Number(e.target.stock.value) });
        e.target.reset();
      }}>
    ...
    if (tabla === "autores") return (
      <form onSubmit={(e) => {
        e.preventDefault();
        crear({ nombre: e.target.nombre.value, nacionalidad: e.target.nacionalidad.value });
        e.target.reset();
      }}>
    ...
    if (tabla === "usuarios") return (
      <form onSubmit={(e) => {
        e.preventDefault();
        crear({ nombre: e.target.nombre.value, tipo_usuario: e.target.tipo.value });
        e.target.reset();
      }}>
    ...
    if (tabla === "prestamos") return (
      <form onSubmit={(e) => {
        e.preventDefault();
        crear({ id_usuario: Number(e.target.usuario.value), id_libro: Number(e.target.libro.value), fecha_prestamo: e.target.fecha.value });
        e.target.reset();
      }}>
    ...
    if (tabla === "empleados") return (
      <form onSubmit={(e) => {
        e.preventDefault();
        crear({ nombre: e.target.nombre.value, apellidos: e.target.apellidos.value, cargo: e.target.cargo.value, telefono: e.target.telefono.value, correo_electronico: e.target.correo_electronico.value });
        e.target.reset();
      }}>
    ...
    <>
      ...
      <header className="header">
        <div className="logo">
          ...
          <span style={{ color: "var(--t3)", marginLeft: 2, fontWeight: 400 }}>PRO</span>
        </div>
        <select
      ...
      <div className="body">
        ...
        <div className="data-panel">
          ...
          {cargando && (
            <p style={{ padding: "16px 20px", color: "var(--t3)", fontSize: ".85rem" }}>Cargando…</p>
          )}
          {error && (
            <p style={{ padding: "10px 20px", color: "var(--danger)", fontSize: ".85rem" }}>⚠ {error}</p>
          )}
            ...
            {Array.isArray(datos) && datos.map((item) => (
              <div key={item.id} className="item-card">
                ...
                <div className="row-actions" style={{ marginTop: 10 }}>
                  <button className="btn-sm btn-edit" onClick={() => abrirEditar(item)}>✏ Editar</button>
